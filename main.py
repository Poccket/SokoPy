...
logging.basicConfig(filename=('logs/sokopy'+str(int(time.time()))+'.log' if args.debug else '/dev/null'), level=logging.DEBUG)
...
def main(stdscr):
	...
	def load_pack(lvlpack):
		...
		while ThirdVariable:
			stdscr.clear()
			real_Benu_items = list(packdata['lvls'].keys()) + ["Back to menu...", ""]
			for index in range(menu_limit):
				itemname = real_Benu_items[(round(sbelect-((menu_limit-modd)/2))+index+4) % len(real_Benu_items)]
				if itemname == "":
					continue
				else:
